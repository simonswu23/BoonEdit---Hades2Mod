...
- **Cherished Heirloom** (Demeter × Hera) — Equip an extra keepsake on pickup. That extra one is a choice rather than an offer: the rack now checks what was actually equipped before it opened, rather than the game's own last-awarded trait (which anything picked up in between could overwrite), so it genuinely cannot be backed out of until you take one, only closing once the screen reports something new is worn. Keepsake effects no longer expire tonight by default — that half is off behind `CherishedHeirloom.KeepAllKeepsakes`. The keepsake you choose through the rack is remembered on the run itself (`CurrentRun.BoonEditHeirloomSpecialKeepsake`) rather than on the trait object, so the bonus survives being unequipped and re-equipped later — and marked out from an ordinary one (`BoonEditHeirloomSpecial`) for the HUD and anything else that wants to check. Re-picking the keepsake you already hold through that rack can refresh it instead of doing nothing — off by default behind `CherishedHeirloom.RefreshHeldKeepsake`, since vanilla's own equip logic silently no-ops when you already hold the trait. A per-keepsake table (`HEIRLOOM_KEEPSAKE_REFRESH`) covers all 33 vanilla keepsakes: most are re-equipped fresh (regranting mana, spell drops, hammers, timers, blessings, and resetting any spent uses, exactly as vanilla's own `AcquireFunctionName`/`SetupFunction` machinery would for a first-time pickup); Gold Purse re-pays its gold directly, since vanilla hands that out from the rack-close code rather than the usual acquire pipeline; Discordant Bell keeps its existing bonus rather than resetting it, scaling the same number of rooms' worth of growth up to whatever rate the refresh landed at; and the nine Force-a-God keepsakes, refreshed while held at Heroic, additionally earn a use that transforms one of that god's own boons: into their Legendary if it isn't already held, else into their Wrath if Wrath of Olympus is installed and it isn't already held, else it falls back to rarifying that boon straight to Heroic. Both bypass the boon's own requirements entirely, since they are granted directly rather than offered. It also waits for the screen to be clear before it opens, rather than counting down a fixed pause — anything that offers a boon on a delay of its own, Concave Stone's random one especially, used to land on top of it.
...
- **Concave Stone** (Echo) — Instead of taking a different random boon, its once-a-night chance now doubles the boon or hammer upgrade you just chose. The game's own chance roll and Echo presentation are reused; vanilla's own "grant something else" roll is switched off for the pick it fires on so the two effects cannot both land off one choice.
- **Calling Card** (Zagreus) — Each use now rarifies straight to Heroic rather than one rarity at a time.
- **White Antler** (Artemis) — No longer restricted to the next region: the crit chance and the health limit it comes with now hold for the rest of the night. Re-applied fresh at every room load once vanilla's own end-of-region reset clears it, using the same restore Cherished Heirloom's refresh uses.
